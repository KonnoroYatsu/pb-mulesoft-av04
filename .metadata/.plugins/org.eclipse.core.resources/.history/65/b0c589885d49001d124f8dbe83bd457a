...
class MappersUtilTest {
    ...
    void deveriaRetornarUmPaymentTypeQuandoReceberUmaStringDePaymentType() {
        PaymentType paymentType = mappersUtil.convertStringToPaymentType("CREDIT_CARD");
        Assert.assertEquals(PaymentType.CREDIT_CARD, paymentType);
        paymentType = mappersUtil.convertStringToPaymentType("credit_card");
        Assert.assertEquals(PaymentType.CREDIT_CARD, paymentType);
    }
    ...
    void deveriaRetornarUmaStringRepresentandoOBodyDaRequisicaoDePagamentoParaPBBank() {
        ...
        Assert.assertTrue(paymenRequestBodyJSON.contains("\"seller_id\":\"7be8890e-f4da-40c2-975e-0b9a87c5ad69\""));
        
        Assert.assertTrue(paymenRequestBodyJSON.contains("\"document_type\":\"CPF\""));
        Assert.assertTrue(paymenRequestBodyJSON.contains("\"document_number\":\"43209756409\""));
        
        Assert.assertTrue(paymenRequestBodyJSON.contains("\"payment_type\":\"CREDIT_CARD\""));
        Assert.assertTrue(paymenRequestBodyJSON.contains("\"currency\":\"BRL\""));
        ...
        Assert.assertTrue(paymenRequestBodyJSON.contains("\"security_code\":\"312\""));        
        Assert.assertTrue(paymenRequestBodyJSON.contains("\"brand\":\"MASTERCARD\""));   
        Assert.assertTrue(paymenRequestBodyJSON.contains("\"expiration_month\":8"));   
        Assert.assertTrue(paymenRequestBodyJSON.contains("\"expiration_year\":\"10\""));   
    }
    
    private List<PaymentEntity> criarPaymentsEntity() {
        List<PaymentEntity> paymentsEntity = new ArrayList<>();
        paymentsEntity.add(new PaymentEntity(12345678910L,
                234.9,"1c3a1fec-9c4b-4b86-a2a2-ff537d29d3c7",
                PaymentStatus.APPROVED,"transaction approved"));

        paymentsEntity.add(new PaymentEntity(12345288910L,
                423.2,"1c3a1fec-4b86-a2a2-ff537d-9c4b-29d3c7",
    ...
    private OrderForm criarOrderForm() {
        ...
        OrderForm orderForm = new OrderForm("43209756409",
                itemsForm, 9.87, 3.43, "CREDIT_CARD",
                "BRL", cardForm);
    ...
    private CardForm criarCardForm() {
        CardForm cardForm = new CardForm("6721546712123",
                "JOAO MURILO", "312", 8, "10", "MASTERCARD");    
        return cardForm;
    }

    private PaymentResponseForm criarPaymentResponseForm() {
        Authorization authorization = new Authorization(
                1659361102747L, "2022-08-01T13:38:32.747325",
                0L, "transaction approved");
        ...
        PaymentResponseForm paymentResponseForm = new PaymentResponseForm(
                ...
                67.9, "BRL","APPROVED", "2022-08-01T13:38:22.747317",
                authorization);
